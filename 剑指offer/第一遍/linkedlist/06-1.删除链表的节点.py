            ...
            删除链表中指定结点的常规方法：
                要分三种情况讨论：
                    1、要删除的结点是头结点 pHead = pHead.next
                        头结点又可以分为：链表只有一个结点（head 和 node 都删除）
                                        和链表有多个结点（head保留到next，node删除）
...
class Solution2:
    def DeleteNode(self, head, node):
        """
            ...
            中删除该结点，其时间复杂度是O(n)。
            如果要达到要求的时间复杂度O(1)，那一定不能再进行遍历操作

            注意到本题不是给定一个数值，删除该数值的结点，而是给出了要删除的结点，就包含要删除结点的下一个节点。
            现在知道了要删除结点，也知道了要删除结点的下一个结点，需要要删除结点的上一个结点，这是未知的。
            我们不知道要删除结点的上个节点，我们知道谁的上结点呢？我们知道要删除结点的下个结点的上个节点，也就是要删除结点，因此可以考虑把要删除的结点的下个节点删掉。
            那我们可以让把下一个结点的值复制给要删除的结点，然后把下一个节点删除

            要删除的结点应该分情况讨论，
                            1）要删除结点是不是尾结点：按上述方法
                            2）要删除结点是尾结点，又可以分为两种情况
                                2-1）要删除结点是头结点，链表中只有一个结点，即 head == node 则将head和node都delete
                                2-2）要删除结点是尾结点，也就是说要删除的结点没有下一个节点，就无法使用上述方法，需要从头遍历链表
